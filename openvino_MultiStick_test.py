...
def camThread(LABELS, results, frameBuffer, camera_width, camera_height, vidfps, cam_input):
    global fps
    global detectfps
    global lastresults
    global framecount
    ...
    while True:
        ...
        k = cv2.waitKey(33)
        if cv2.waitKey(wait_key_time)& k == 27:
            break
...
class NcsWorker(object):
    ...
    def __init__(self, devid, frameBuffer, results, camera_width, camera_height, number_of_ncs, vidfps, tiny, size):
        self.devid = devid
        ...
        self.roop_frame = 0
        self.vidfps = vidfps
        self.new_w = size #int(camera_width * min(self.m_input_size/camera_width, self.m_input_size/camera_height))
        self.new_h = size #int(camera_height * min(self.m_input_size/camera_width, self.m_input_size/camera_height))
...
def inferencer(results, frameBuffer, number_of_ncs, camera_width, camera_height, vidfps, tiny, size):

    # Init infer threads
    threads = []
    for devid in range(number_of_ncs):
        thworker = threading.Thread(target=async_infer, args=(NcsWorker(devid, frameBuffer, results, camera_width, camera_height, number_of_ncs, vidfps, tiny, size),))
        thworker.start()
...
if __name__ == '__main__':
    ...
    try:
        ...
        p_inf = mp.Process(target=inferencer, args=(results, frameBuffer, number_of_ncs, camera_width, camera_height, vidfps, args.tiny, args.size), daemon=True)
        p_inf.start()
        processes.append(p_inf)

        sleep(number_of_ncs * 7)

        # Start streaming
        p_cam = mp.Process(target=camThread, args=(LABELS, results, frameBuffer, camera_width, camera_height, vidfps, args.video), daemon=True)
        p_cam.start()
